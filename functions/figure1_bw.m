...
function h = figure1_bw(y,theta)

  % Color/Line Specifications 
  blk = [0, 0, 0];
  gry = [2/3, 2/3, 2/3];
  ColorArray = {blk,blk,blk,gry,gry,gry}';
  LineArray = {'-','--',':','-','--',':'}';
  ...
  % Create axes 
    ...
    hold(axes1,'on');
    sub = plot(theta',[y(:,1,1),y(:,1,2),y(:,1,3),y(:,1,4),y(:,1,5),y(:,1,6)],...
      'LineWidth',2.5); grid on;
    set(sub,{'Color'},ColorArray,{'LineStyle'},LineArray);
  ...
  % Create axes 
   ...
   hold(axes2,'on');
   sub = plot(theta',[y(:,2,1),y(:,2,2),y(:,2,3),y(:,2,4),y(:,2,5),y(:,2,6)],...
      'LineWidth',2.5); grid on;
    set(sub,{'Color'},ColorArray,{'LineStyle'},LineArray);
    xlabel('CRRA (\theta)'); ylabel('Lobby Effort (L)');
    ylim([0 30])
  ...
  % Create axes 
   ...
   hold(axes3,'on');
   sub = plot(theta',[y(:,3,1),y(:,3,2),y(:,3,3),y(:,3,4),y(:,3,5),y(:,3,6)],...
      'LineWidth',2.5); grid on;
    set(sub,{'Color'},ColorArray,{'LineStyle'},LineArray);
    xlabel('CRRA (\theta)'); ylabel('Lobby Effort (L)');
    ylim([0 30])
