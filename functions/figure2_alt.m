...
function g = figure2_alt(y,theta)
...
g = figure('Name','Nash Equilibrium Lobbying Efforts by Treatment',...
  ...
  % Create axes 
    ...
    hold(axes1,'on');
    sub = plot(theta',[y(:,1,1),y(:,2,1),y(:,3,1),y(:,3,1)],'LineWidth',2); 
    grid on;
    set(sub,{'color'},{[0, 0.4470, 0.7410],[0.8500, 0.3250, 0.0980],...
        [0.9290, 0.6940, 0.1250],[0.25, 0.25, 0.25]'}');
  ...
  % Create axes 
    ...
    hold(axes2,'on');
    sub = plot(theta',[y(:,1,2),y(:,2,2),y(:,3,2),y(:,3,2)],'LineWidth',2); 
    grid on;
    set(sub,{'color'},{[0, 0.4470, 0.7410],[0.8500, 0.3250, 0.0980],...
        [0.9290, 0.6940, 0.1250],[0.25, 0.25, 0.25]'}');
    xlabel('CRRA (\theta)'); ylabel('Lobby Effort (L)');
  ...
  % Create axes 
    ...
    hold(axes3,'on');
   sub = plot(theta',[y(:,1,3),y(:,2,3),y(:,3,3),y(:,3,3)],'LineWidth',2); 
    grid on;
    set(sub,{'color'},{[0, 0.4470, 0.7410],[0.8500, 0.3250, 0.0980],...
        [0.9290, 0.6940, 0.1250],[0.25, 0.25, 0.25]'}');
    xlabel('CRRA (\theta)'); ylabel('Lobby Effort (L)');
